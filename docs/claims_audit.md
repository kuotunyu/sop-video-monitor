...
| robustness to unseen error types | no | `docs/what_this_does_not_show.md` "Synthetic violations are not real violations"; SOP reports keep synthetic and native tables apart | HA-ViD native `wrong`: 67 segments, 16 in the test subjects, 12 in val, 0 detected (`reports/havid_audit.md`, `havid_dev_v8`) — the README table's "HA-ViD `wrong` 段數未公布" is now measured, and the permitted wording still applies |
...
- HA-ViD results are qualified as "development result on `splits/ha-vid/val.csv`" and never as
  held-out test results; the permitted phrase "on HA-ViD's fixed three views, held-out subjects"
  may be used only after a frozen test-split evaluation exists.
...
- Evaluate the frozen test splits once, after the protocol is fixed (requires an explicit
  decision), and only then use "held-out subjects" wording for HA-ViD.
