...
# `make reproduce` is the full data + GPU path. Targets are grouped by line; each `##` comment is
# the one-line description. Development targets write under reports/; the test target refuses to
# run twice.

.PHONY: reproduce-lite test lint verify-splits audit audit-havid freeze-havid extract-psr features features-vits learn-sop psr psr-latency baseline mstcn psr-lopo psr-train psr-nested features-havid i3d-havid havid-tas havid-tas-v3 havid-tas-i3d havid-tas-seeds havid-seeds-summary havid-tas-lookahead havid-lookahead-summary havid-step-recall-v9 learn-havid-sop havid-sop tune-havid-sop havid-sop-v5 havid-sop-v6 tune-havid-sop-sheet havid-sop-v8 havid-online-v10 havid-final-L0 havid-final-lstar havid-test stream-bench-decode stream-bench-dinov2 review-queue review-ui figures sop-knowledge-doc reproduce
...
# ---- Data: audits, frozen splits, label extraction (needs the local copies, see data/README.md) ---
...
# ---- IndustReal, historical protocols kept so the older reports stay reproducible ------------
...
# ---- HA-ViD recogniser (development results on val) ------------------------------------------
...
# ---- HA-ViD SOP layer (development results on val) -------------------------------------------
...
# ---- HA-ViD final networks and the one-time test (docs/havid_test_protocol.md; the test is spent) ---
...
# ---- Streaming benchmark (needs an otherwise idle machine) -----------------------------------
...
# ---- Full data + GPU path in dependency order ------------------------------------------------
